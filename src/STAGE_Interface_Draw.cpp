...
void STAGE_Interface_Draw()
{
  ...
  {
    // High-density 1-line Cardputer ZERO HUD
    Print(HUD.PlayerX, HUD.PlayerY, 0, 0, "GIANA");
    char scoreStr[16];
    ...
    char livesStr[8];
    snprintf(livesStr, sizeof(livesStr), "x%02d", PC.Lives);
    Print(HUD.LivesX + 12, HUD.LivesY, 3, 0, livesStr);

    char stageStr[16];
    snprintf(stageStr, sizeof(stageStr), "STG %d", PC.Stage);
