...
void GAMETYPE_Load()
{
  ...
  std::string c64Dir = GetAssetPath("c64");
  std::string defDir = GetAssetPath("default");
  std::string amigaDir = GetAssetPath("amiga");

  DIR *dip;
  if ((dip = opendir(c64Dir.c_str())) != NULL){closedir(dip); GV.DirC64Exists = true;}
  if ((dip = opendir(defDir.c_str())) != NULL){closedir(dip); GV.DirDefaultExists = true;}
  if ((dip = opendir(amigaDir.c_str())) != NULL){closedir(dip); GV.DirAmigaExists = true;}
  // IF DIRECTORIES DON'T EXIST, FALL BACK ONTO C64
  ...
  if(!GV.DirC64Exists && GV.DirDefaultExists){GV.GameType = TYPE_OPENGGS;}
