...
void STAGES_Import()
{
  ...
  std::string stagePath = GetAssetPath(FileName.Stage[0]);
  FILE *Stages_File = fopen (stagePath.c_str(), "rb");
...
void STAGES_Import_RANDOM_Modules()
{
  ...
  std::string modPath = GetAssetPath("stages/modules.lvl");
  FILE *Stages_File = fopen (modPath.c_str(), "rb");
